...
if(isset($_GET['action'])){
	...
	$id=$_GET['id'];
	try {
		$stmt = $conexao->prepare("SELECT usuarioid,categoria FROM inventario WHERE id=?");
		$stmt->execute([$id]);
	...
	vn($id);
	try {
		$stmt = $conexao->prepare("UPDATE inventario SET status='off' WHERE usuarioid=? AND categoria=?");
...
try {
	$stmt = $conexao->prepare("SELECT i.id,i.status,i.upgrade,t.categoria,t.descricao,t.taijutsu,t.ninjutsu,t.genjutsu,t.nome,t.imagem,t.valor FROM inventario i LEFT OUTER JOIN table_itens t ON i.itemid=t.id WHERE i.usuarioid=? AND venda='nao' ORDER BY status ASC");
	$stmt->execute([$db['id']]);
	$sqli = $stmt;
	$dbi = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
...
<div class="box_middle">Abaixo está todo seu inventário. Estarão listados aqui todos os itens que existirem em sua conta, incluindo os equipamentos equipados e não-equipados. Lembre-se que ao equipar um certo tipo de item, qualquer outro que seja do mesmo tipo será automaticamente substituído. Você pode também vender o item para o comércio de sua vila, ou então anunciar em sua <a href="?p=myshop">própria loja</a>, para que outros jogadores o comprem.<div class="sep"></div>
    ...
    <?php if((!$sqlr || $sqlr->rowCount()==0) && (!$sqli || $sqli->rowCount()==0)){ ?>
    <div class="sep"></div>
