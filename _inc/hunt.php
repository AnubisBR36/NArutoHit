...
if(isset($_POST['hunt_tipo'])){
	...
	switch($tipo){
		case 1:
			...
			$nome=$hunt;
			try {
				$sqlh = $conexao->prepare("SELECT id,vila,renegado,avatar,energia,preso,penalidade_fim,missao,loginip,tipo FROM usuarios WHERE usuario=?");
				$sqlh->execute([$nome]);
		...
		case 2:
			...
			try {
				if($hunt==7) {
					$sqlh = $conexao->prepare("SELECT id,energia,preso FROM usuarios WHERE avatar>0 AND energia>=25 AND renegado='sim' AND id<>? AND missao<>999 AND loginip<>? ORDER BY RANDOM() LIMIT 1");
					$sqlh->execute([$db['id'], $db['loginip']]);
				} else {
					$sqlh = $conexao->prepare("SELECT id,energia,preso FROM usuarios WHERE avatar>0 AND energia>=25 AND vila=? AND renegado='nao' AND id<>? AND missao<>999 AND loginip<>? ORDER BY RANDOM() LIMIT 1");
					$sqlh->execute([$vila, $db['id'], $db['loginip']]);
				}
				$dbh = $sqlh->fetch(PDO::FETCH_ASSOC);
			} catch (PDOException $e) {
			...
			if($vila==7){
				...
				if(($dbh['vila']==$db['vila'])&&($dbh['renegado']=='nao')&&($dbh['renegado']=='nao')){ echo "<script>self.location='?p=hunt&msg=11'</script>"; break; }
			}
			$_SESSION['prepare']=$dbh['id'];
			$_SESSION['hunt']=2;
			echo "<script>self.location='?p=prepare'</script>"; break;
		case 3:
			...
			try {
				if($db['renegado']=='nao') {
					$sqlh = $conexao->prepare("SELECT id,energia,preso FROM usuarios WHERE avatar>0 AND energia>=25 AND ".$filtro." AND vila<>? AND id<>? AND missao<>999 ORDER BY RANDOM() LIMIT 1");
					$sqlh->execute([$db['vila'], $db['id']]);
				} else {
					$sqlh = $conexao->prepare("SELECT id,energia,preso FROM usuarios WHERE avatar>0 AND energia>=25 AND ".$filtro." AND renegado='nao' AND id<>? AND missao<>999 ORDER BY RANDOM() LIMIT 1");
					$sqlh->execute([$db['id']]);
				}
				$dbh = $sqlh->fetch(PDO::FETCH_ASSOC);
			} catch (PDOException $e) {
		...
		case 5:
			...
			try {
				if($vilaid==9) {
					$sqlh = $conexao->prepare("SELECT id,energia,preso,vila,renegado,penalidade_fim FROM usuarios WHERE energia>=25 AND avatar>0 AND id<>? AND missao<>999 AND loginip<>? ".$tempo." AND renegado='sim' ORDER BY RANDOM() LIMIT 1");
					$sqlh->execute([$db['id'], $db['loginip']]);
				} else {
					$sqlh = $conexao->prepare("SELECT id,energia,preso,vila,renegado,penalidade_fim FROM usuarios WHERE energia>=25 AND avatar>0 AND id<>? AND missao<>999 AND loginip<>? ".$tempo." AND renegado='nao' AND vila=? ORDER BY RANDOM() LIMIT 1");
					$sqlh->execute([$db['id'], $db['loginip'], $vilaid]);
				}
				$dbh = $sqlh->fetch(PDO::FETCH_ASSOC);
			} catch (PDOException $e) {
				echo "<script>self.location='?p=hunt&msg=5'</script>"; break;
