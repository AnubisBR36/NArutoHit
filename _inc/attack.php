...
if($db['doujutsu']==2){ $txtdoujutsu1='Byakugan'; $addtai1=round($db['taijutsu']*($db['doujutsu_nivel']/50)); } else $addtai1=0;
if($db['doujutsu']==3){ $txtdoujutsu1='Rinnegan'; $addnin1=round($db['ninjutsu']*($db['doujutsu_nivel']/50)); } else $addnin1=0;
if($db['doujutsu']==1){ $txtdoujutsu1='Sharingan'; $addgen1=round($db['genjutsu']*($db['doujutsu_nivel']/50)); } else $addgen1=0;
if($db['orgnivel']>0){
...
if($dbi['doujutsu']==2){ $txtdoujutsu2='Byakugan'; $addtai2=round($dbi['taijutsu']*($dbi['doujutsu_nivel']/50)); } else $addtai2=0;
if($dbi['doujutsu']==3){ $txtdoujutsu2='Rinnegan'; $addnin2=round($dbi['ninjutsu']*($dbi['doujutsu_nivel']/50)); } else $addnin2=0;
if($dbi['doujutsu']==1){ $txtdoujutsu2='Sharingan'; $addgen2=round($dbi['genjutsu']*($dbi['doujutsu_nivel']/50)); } else $addgen2=0;
if($dbi['orgnivel']>0){
	...
	<table width="100%" cellpadding="0" cellspacing="0">
    	...
    	<tr>
        	<td colspan="2" align="center" width="42%" style="background:url(_img/personagens/no_avatar.jpg) no-repeat center #444444;">
                <img src="_img/personagens/<?php echo $db['personagem']; ?>/<?php echo $db['avatar']; ?>.jpg" onmouseover="Tip('<div class=tooltip><?php echo fpersonagem($db['personagem']); ?></div>')" onmouseout="UnTip()" />
                <?php if(is_top1_vila($db['id'], $db['vila']) || true): ?>
                    <div style="background:#444444; padding:2px;">
                        <?php if(is_top1_vila($db['id'], $db['vila'])): ?>
                            <img src="_img/Chapeu/bg-kage-konoha.jpg" style="width:60px; height:auto; display:block; margin:1px auto;" />
                        <?php endif; ?>
                        <div style="position:relative; display:inline-block; margin:1px auto;">
                            <img src="_img/Chapeu/Pergaminho.jpg" style="width:80px; height:25px; display:block;" />
                            <div style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); color:#FFD700; font-weight:bold; font-size:10px; text-shadow: -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000; font-family:'Comic Sans MS', cursive, fantasy; letter-spacing:0.5px;">
                                <?php 
                                // Get nivel text for pergaminho
                                if($db['renegado']=='sim'){
                                ...
                                echo strtoupper($nivel_pergaminho);
                                ?>
                            </div>
                        </div>
                    </div>
            ...
            <td colspan="2" align="center" width="42%" style="background:url(_img/personagens/no_avatar.jpg) no-repeat center #444444;">
                <img src="_img/personagens/<?php echo $dbi['personagem']; ?>/<?php echo $dbi['avatar']; ?>.jpg"onmouseover="Tip('<div class=t wiooltip><?php echo fpersonagem($dbi['personagem']); ?></div>')" onmouseout="UnTip()" />
                <?php if(is_top1_vila($dbi['id'], $dbi['vila']) || true): ?>
                    <div style="background:#444444; padding:2px;">
                        <?php if(is_top1_vila($dbi['id'], $dbi['vila'])): ?>
                            <img src="_img/Chapeu/bg-kage-konoha.jpg" style="width:60px; height:auto; display:block; margin:1px auto;" />
                        <?php endif; ?>
                        <div style="position:relative; display:inline-block; margin:1px auto;">
                            <img src="_img/Chapeu/Pergaminho.jpg" style="width:80px; height:25px; display:block;" />
                            <div style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); color:#FFD700; font-weight:bold; font-size:10px; text-shadow: -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000; font-family:'Comic Sans MS', cursive, fantasy; letter-spacing:0.5px;">
                                <?php 
                                // Get nivel text for pergaminho for enemy
                                if($dbi['renegado']=='sim'){
                                    $sqlx=mysql_query("SELECT id FROM usuarios WHERE renegado='sim' ORDER BY nivel DESC, yens_fat DESC, vitorias DESC, derrotas ASC LIMIT 1");
                                    $dbx=mysql_fetch_assoc($sqlx);
                                    if($dbx && $dbx['id']==$dbi['id']) $nivel_pergaminho='Líder da Akatsuki'; else $nivel_pergaminho='Nukenin';
                                } else {
                                    $sqlx=mysql_query("SELECT id FROM usuarios WHERE vila=".$dbi['vila']." AND renegado='nao' ORDER BY nivel DESC, yens_fat DESC, vitorias DESC, derrotas ASC LIMIT 1");
                                    $dbx=mysql_fetch_assoc($sqlx);
                                    if($dbx && $dbx['id']==$dbi['id']){
                                        switch($dbi['vila']){
                                            case 1: $nivel_pergaminho='Hokage'; break;
                                            case 2: $nivel_pergaminho='Kazekage'; break;
                                            case 3: $nivel_pergaminho='Otokage'; break;
                                    ...
                                    } else $nivel_pergaminho=rankNinja($dbi['nivel']); 
                                }
                                echo strtoupper($nivel_pergaminho);
                                ?>
                            </div>
        ...
        <tr>
        	<td colspan="2" bgcolor="#444444" align="center"><img src="_img/NewsVilas/<?php if($db['renegado']=='sim') echo 'akatsuki'; else echo $vila; ?>.jpg" onmouseover="Tip('<div class=tooltip><?php echo $txtvila; ?></div>');" onmouseout="UnTip()" /></td>
            <td align="center">&nbsp;</td>
            <td colspan="2" bgcolor="#444444" align="center"><img src="_img/NewsVilas/<?php if($dbi['renegado']=='sim') echo 'akatsuki'; else echo $vilai; ?>.jpg" onmouseover="Tip('<div class=tooltip><?php echo $txtvilai; ?></div>');" onmouseout="UnTip()" /></td>
        </tr>
        <tr>
        	<td colspan="2" align="center"><div class="sep"></div></td>
            <td align="center"></td>
...
<div class="box_middle">
	...
	if($a==1){
		...
		mysql_query("UPDATE usuarios SET yens=yens+$yens, yens_fat=yens_fat+$yens, exp=exp+$exp_1, exptotal=exptotal+$exp_1, penalidade_fim='$penalidade', vitorias=vitorias+1, quest_vitorias=quest_vitorias+1, batalhas=batalhas+1, doujutsu_exp=doujutsu_exp+$expd_1, energia=".$db['energia']." WHERE id=".$db['id']);
	...
	if($a==2){
		...
		mysql_query("UPDATE usuarios SET yens=yens+$yens, yens_fat=yens_fat+$yens, exp=exp+$exp_2, exptotal=exptotal+$exp_2, vitorias=vitorias+1, quest_vitorias=quest_vitorias+1, batalhas=batalhas+1, doujutsu_exp=doujutsu_exp+$expd_2, energia=".$dbi['energia']." WHERE id=".$dbi['id']);
	} else
	if($a==3){
		$vencedorid=0;
		$vencedor='Empate';
	...
	if($dbi['tipo']=='player'){
		mysql_query("INSERT INTO relatorios (data, usuarioid, inimigoid, vencedor, exp, yens, taijutsu, ninjutsu, genjutsu, energia, equips1, equips2, danos, nivel, doujutsu, ip) VALUES ('".date('Y-m-d H:i:s')."', ".$db['id'].", ".$dbi['id'].", $vencedorid, '$exp', $yens, '$tai', '$nin', '$gen', '$ene', '$equips1', '$equips2', '$danos', '$nivel', '$reldoujutsu', '$ip')");
