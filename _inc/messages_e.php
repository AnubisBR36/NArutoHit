...
try {
    ...
    $stmt_messages = $conexao->prepare("SELECT m.*, u.usuario FROM mensagens m LEFT OUTER JOIN usuarios u ON m.destino = u.id WHERE m.origem = ? ORDER BY m.data DESC LIMIT $min, 10");
...
<table width="100%" cellpadding="0" cellspacing="1">
  ...
  <tr style="background:#323232;" onmouseover="style.background='#2C2C2C'" onmouseout="style.background='#323232'">
    <td align="center" width="30"><img src="_img/unread.png" /></td>
    ...
    <td style="padding-left:5px;"><?php if($dbmm['destino']==0) echo '<b>narutoHIT</b>'; else { ?><a href="?p=view&amp;view=<?php echo strtolower($dbmm['usuario']); ?>"><?php echo $dbmm['usuario']; ?></a><?php } ?><br /><span class="sub2"><b>Assunto:</b> <?php echo $dbmm['assunto']; ?></span></td>
    <td align="center" width="100"><a href="search_msg.php?id=<?php echo $dbmm['id']; ?>&amp;key=<?php echo $c->encode($db['id'],$chaveuniversal); ?>" class="modal" rel="modal">Ver</a> | <a href="?p=messages&amp;type=e&amp;pg=<?php echo $pg; ?>&amp;del=<?php echo $dbmm['id']; ?>">Apagar</a></td>
  </tr>
  <?php $i++; } } ?>
  <?php if($qt>10){ ?>
  <tr style="background:#323232;">
