...
$dif=$db['taijutsu']+7;
try {
    $sqls = $conexao->prepare("SELECT * FROM table_itens WHERE categoria='arma' AND reqtai<? ORDER BY reqtai ASC");
    $sqls->execute([$dif]);
...
<table width="100%" cellpadding="0" cellspacing="1">
    <?php if(!$sqls || $sqls->rowCount()==0) echo '<tr><td colspan="3"><div class="aviso">Nenhum item encontrado.</div></td></tr>'; else do{ if(date('Y-m-d H:i:s')<$db['vip']) $dbs['valor']=$dbs['valor']-($dbs['valor']*0.15); ?>
