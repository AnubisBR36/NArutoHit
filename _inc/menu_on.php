...
<?php } ?>
<div align="center" style="background:url(_img/personagens/no_avatar.jpg) no-repeat top;height:150px; position:relative;">
    ...
    <?php
    // Verificar mensagens não lidas para notificação no avatar
    try {
        ...
        if($unread_avatar && $unread_avatar['total'] > 0) {
            echo '<div style="position:absolute; top:5px; right:5px; background:#ff0000; color:#ffffff; border-radius:10px; padding:2px 6px; font-size:10px; font-weight:bold; min-width:15px; text-align:center;">';
...
if($db['renegado']=='sim'){
    ...
    $stmt = $conexao->prepare("SELECT id FROM usuarios WHERE vila=? AND renegado='nao' ORDER BY nivel DESC, yens_fat DESC, vitorias DESC, derrotas ASC LIMIT 1");
