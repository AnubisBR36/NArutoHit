...
if(!isset($subaction) or $subaction == ''){ $subaction = $_POST['subaction'] ?? ''; }
...
if(!isset($template) or $template == "" or strtolower($template) == "default"){ require_once("$cutepath/data/Default.tpl"); }
    ...
    else{ die("Error!<br>the template <b>".htmlspecialchars($template)."</b> does not exists, note that templates are case sensitive and you must write the name exactly as it is"); }
}
...
$CN_HALT = $CN_HALT ?? false;
$static = $static ?? false;

// --------- Determine what user wants to do
if($CN_HALT != TRUE and $static != TRUE and ($subaction == 'showcomments' or $subaction == 'showfull' or $subaction == 'addcomment') and ((!isset($category) or $category == '') or (isset($requested_cats[$ucat]) && $requested_cats[$ucat] == TRUE )  ) ){
